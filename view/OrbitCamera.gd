...
func update_focus(simulation: Simulation) -> void:
	if focus_target_id < 0:
		return

	match focus_target_type:
		"body":
			pivot_point = simulation.get_body_position(focus_target_id)
		"station":
			pivot_point = simulation.get_station_position(focus_target_id)
		"ship":
			if focus_target_id in simulation.world.ships:
				pivot_point = simulation.world.ships[focus_target_id].position
	_update_camera_transform()
